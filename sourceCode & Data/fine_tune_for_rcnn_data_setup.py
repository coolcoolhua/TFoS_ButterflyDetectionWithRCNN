...
def writeRCNN(sc, input , output, format, num_partitions):
  """Writes our image/label vectors into parallelized files on HDFS"""
 ...
 # with open(input_labels, 'rb') as f:
  ...
  images,labels = pickle.load(open(input, 'rb'))
...
if __name__ == "__main__":
  ...
  parser.add_argument("--format", help="output format", choices=["csv", "csv2", "pickle", "tf", "tfr"], default="pickle")
  parser.add_argument("--num-partitions", help="Number of output partitions", type=int, default=10)
  parser.add_argument("--output", help="HDFS directory to save examples in parallelized format", default="datasetTFrecord/train1")
  parser.add_argument("--input", help="HDFS directory to save examples in parallelized format", default="/home/hduser/1dataset.pkl")
  ...
  sc = SparkContext(conf=SparkConf().setAppName("rcnn_data_parallelize"))
  ...
  if not args.read:
   ...
   #writeMNIST(sc, "mnist/train-images-idx3-ubyte.gz", "mnist/train-labels-idx1-ubyte.gz", args.output + "/train", args.format, args.num_partitions)
    writeRCNN(sc,args.input, args.output , args.format, args.num_partitions)
